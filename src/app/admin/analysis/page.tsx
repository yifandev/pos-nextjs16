...
import { getSalesAnalytics, AnalyticsFilters as FilterType } from "@/actions/analytics.actions";
...
import { AnalyticsFilters, PeriodFilter } from "@/components/features/analytics/analytics-filters";
...
export default function AnalysisPage() {
  ...
  const handleExportExcel = () => {
    ...
    try {
      ...
      // Overview sheet
      const overviewData = [
        { Metric: 'Total Pendapatan', Value: formatCurrency(data.overview.totalRevenue) },
        { Metric: 'Total Penjualan', Value: formatCurrency(data.overview.totalSales) },
        { Metric: 'Total Transaksi', Value: data.overview.totalTransactions },
        { Metric: 'Rata-rata Transaksi', Value: formatCurrency(data.overview.averageTransaction) },
        { Metric: 'Total Pajak', Value: formatCurrency(data.overview.totalTax) },
      ];
      ...
      // Save file
      XLSX.writeFile(wb, `analytics-${period}-${new Date().toISOString().split('T')[0]}.xlsx`);
    ...
    try {
      ...
      autoTable(doc, {
        ...
        body: [
          ...
          ['Rata-rata Transaksi', formatCurrency(data.overview.averageTransaction)],
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Dashboard analisis penjualan dan performa</p>
        </div>
